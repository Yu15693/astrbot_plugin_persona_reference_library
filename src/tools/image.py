    ...
    for component in event.get_messages():
        ...
        if raw.startswith("base64://"):
            try:
                ...
                logger.exception("Failed to convert input base64 image to data URL.")
            continue
        try:
        ...
        try:
            ...
            logger.exception("Failed to convert normalized base64 image to data URL.")
    return images
...
def build_image_send_result(event: AstrMessageEvent, image: AdapterImage):
    ...
    logger.warning("Unsupported output image kind: %s", image.kind)
    return None
